...
// char MQTT_USER = '';     // MQTT broker user login
// char MQTT_PASS = '';    // MQTT broker password
...
const long mapping[MappingRow][2] = {
  // ms > m/s
  { 987654321, 0 },
  { 120, 1 },
  { 50, 2 },
...
// mDNS
  ...
  if (!rf69.init()) {
    ...
    if (!rf69.setFrequency(RF69_FREQ)) {
      ...
      uint8_t key[] = { 0xDA, 0x2C, 0x23, 0x6B, 0xF5, 0xCF, 0xF0, 0xE8,
                        0x0F, 0xCE, 0xBD, 0xCD, 0xAA, 0x12, 0x53, 0xA4 };
...
void AprsWxIgate() {
  if (AprsON == true) {
    if (!AprsClient.connect("euro.aprs2.net", 14580)) {  // client.connect(URL, port);  char URL[]="google.com"
      if (EnableSerialDebug > 0) {
    ...
    // send data
    if (EnableSerialDebug > 0) {
      Prn(3, 1, "APRS-TX | " + YOUR_CALL + ">APRSWX,TCPIP*,qAS,:=" + AprsCoordinates + "_" + LeadingZero(3, WindDir) + "/" + LeadingZero(3, WindSpeedAvgMPS / 0.447) + "g" + LeadingZero(3, WindSpeedMaxPeriodMPS / 0.447) + "t" + LeadingZero(3, (TemperatureCelsius)*1.8 + 32) + "h" + LeadingZero(3, constrain(HumidityRelPercent, 0, 100)) + "b" + LeadingZero(6, PressureHPA * 100) + "P" + LeadingZero(3, RainTodayMM / 0.254));
    }
    ...
    if (EnableSerialDebug > 0) {
      ...
      Prn(3, 1, "APRS-TX | " + YOUR_CALL + ">APRSWX,TCPIP*,qAC:> WiSE Weather Station");
    }
    AprsClient.println(YOUR_CALL + ">APRSWX,TCPIP*,qAC:> WiSE Weather Station");
  }
}
    ...
    while (webClient.connected()) {
      if (webClient.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          webClient.println(F("          <link rel=\"stylesheet\" type=\"text/css\" href=\"https://isemann.family/style.css\">"));
          // TITLE
          webClient.print(F("           <title>OE3XKU WiSE Weather Station "));
          webClient.print(YOUR_CALL);
          webClient.println(F("</title>"));
          // END TITLE
          webClient.println(F("          <link rel=\"apple-touch-icon\" sizes=\"180x180\" href=\"https://isemann.family/apple-touch-icon.png\">"));
          webClient.println(F("          <link rel=\"mask-icon\" href=\"https://isemann.family/safari-pinned-tab.svg\" color=\"#5bbad5\">"));
          webClient.println(F("          <link rel=\"icon\" type=\"image/png\" href=\"https://isemann.family/favicon-32x32.png\" sizes=\"32x32\">"));
          webClient.println(F("          <link rel=\"icon\" type=\"image/png\" href=\"https://isemann.family/favicon-16x16.png\" sizes=\"16x16\">"));
          webClient.println(F("          <link rel=\"manifest\" href=\"https://isemann.family/manifest.json\">"));
          webClient.println(F("          <link rel=\"shortcut icon\" href=\"https://isemann.family/favicon.ico\">"));
          ...
          webClient.println(F("          <meta name=\"msapplication-config\" content=\"https://isemann.family/browserconfig.xml\">"));
          ...
          webClient.println(ETH.localIP());
          webClient.print(F(":82/update\" target=_blank>Upload FW</a> | <a href=\"https://github.com/isemann/WiSE-Weather-Station\" target=_blank>Releases</a> | <a href=\"http://"));
          webClient.println(ETH.localIP());
          ...
          webClient.println(F("                      OE3XKU Client <code id=\"status-client\" title=\"Client ID\">?</code> is "));
          ...
          webClient.println(F("                      <em>via</em> MQTT Wall"));
          webClient.println(F("                      | <a href=\"https://isemann.at\" target=\"_blank\">OE3BIA, OE9RWV, OE3JSI</a>."));
          webClient.println(F("                  </p>"));
          webClient.println(F("              </div>"));
          webClient.println(F("          </div>"));
          ...
          webClient.println(F("          <script type=\"text/javascript\" src=\"https://isemann.family/mqtt-wall/wall.js\"></script>"));
          webClient.println(F("      </body>"));
          webClient.println(F("  </html>"));
...
void EthEvent(WiFiEvent_t event) {
  switch (event) {
    ...
    case ARDUINO_EVENT_ETH_START:
      ...
      ETH.setHostname("WiSE Weather Station");
      break;
...
void handleSet() {
  ...
  switch (BaudRate) {
    case 1200:
      {
        baudSELECT0 = " selected";
        break;
      }
    case 2400:
      {
        baudSELECT1 = " selected";
        break;
      }
    case 4800:
      {
        baudSELECT2 = " selected";
        break;
      }
    case 9600:
      {
        baudSELECT3 = " selected";
        break;
      }
    case 115200:
      {
        baudSELECT4 = " selected";
        break;
      }
  }
